...
import ashlib.data.plugins.ui.models.StatNumberUIPackage;
...
import static ashlib.data.plugins.misc.AshMisc.getStringFromStatFloat;
...
public class ShipInfoGenerator {
    ...
    private static ArrayList<StatNumberUIPackage> getCombatData(FleetMemberAPI ship, TooltipMakerAPI tooltipSecondRowOfLogistics) {
        ...
        text1 = AshMisc.getNumberString(ship.getStats().getArmorBonus().computeEffective(ship.getHullSpec().getArmorRating())) + "";
        ...
        if (type.equals(ShieldAPI.ShieldType.NONE)) {
            ...
            text1 =  AshMisc.getNumberString(ship.getStats().getPhaseCloakActivationCostBonus().computeEffective(ship.getHullSpec().getShieldSpec().getPhaseCost())) + "";
            ...
            text1 =  AshMisc.getNumberString( ship.getStats().getShieldArcBonus().computeEffective(shieldSpecAPI.getArc())) + "";
            ...
            text1 =   AshMisc.getNumberString(shieldSpecAPI.getUpkeepCost() * ship.getStats().getShieldUpkeepMult().getModifiedValue()) + "";
            ...
            text1 = AshMisc.getNumberString(shieldSpecAPI.getFluxPerDamageAbsorbed() * ship.getStats().getShieldDamageTakenMult().getModifiedValue());
    ...
    private static String getStringFromStat(float base, float modified) {
        ...
        if (diff < 0) {
            return "(+" + AshMisc.getNumberString(Math.abs(diff)) + ")";
        } else {
            return "(-" +  AshMisc.getNumberString(Math.abs(diff)) + ")";
        }
    }
    ...
    private static Pair<HashMap<CustomPanelAPI, ShipRenderInfo.Module>, CustomPanelAPI> generateImageOfModularShip(ShipHullSpecAPI specAPI, CustomPanelAPI panelHolder, ShipRenderInfo info, double scale) {
        CustomPanelAPI centralPanel = panelHolder.createCustomPanel((float) ((float) info.width * scale), (float) ((float) info.height * scale), null);
        double translatedY = (float) (info.height * scale - info.center.getY() * scale);


        double posX = info.center.x * scale;
        double posY = translatedY;
        ShipRenderInfo.Module centralModule = info.getCentralModule();
        ...
        for (ShipRenderInfo.Module module : info.moduleSlotsOnOriginalShip) {
            CustomPanelAPI panel = centralPanel.createCustomPanel((float) ((float) module.width * scale), (float) ((float) module.height * scale), null);
            double translatedYMod = module.height * scale - module.center.getY() * scale;
            double posXMod = -(module.center.x) * scale;
            double posYMod = -(translatedYMod);

            double centerX = -module.slotOnOriginal.locationOnShip.getX() * scale;
            double centerY = -module.slotOnOriginal.locationOnShip.getY() * scale;
            centralPanel.addComponent(panel).inTL((float) (centerX + posX + posXMod), (float) (centerY + posY + posYMod));
    ...
    private static Pair<HashMap<CustomPanelAPI, ShipRenderInfo.Module>, CustomPanelAPI> generateImageOfModularShipSized(ShipHullSpecAPI specAPI, CustomPanelAPI panelHolder, ShipRenderInfo info, double scale, float totalWidth, float totalHeight) {
        CustomPanelAPI mainCentralPanel = panelHolder.createCustomPanel(totalWidth, totalHeight, null);
        CustomPanelAPI centralPanel = mainCentralPanel.createCustomPanel((float) ((float) info.width * scale), (float) ((float) info.height * scale), null);

        double translatedY = (float) (info.height * scale - info.center.getY() * scale);
        double posX = info.center.x * scale;
        double posY = translatedY;
        double furtherstXLeft = 0f;
        double furtherstYTop = 0f;
        double furtherstXRight = 0f;
        double furtherstYBot = 0f;
        ...
        for (ShipRenderInfo.Module module : info.moduleSlotsOnOriginalShip) {
            CustomPanelAPI panel = centralPanel.createCustomPanel((float) ((float) module.width * scale), (float) ((float) module.height * scale), null);
            double translatedYMod = (float) (module.height * scale - module.center.getY() * scale);
            double posXMod = -(module.center.x) * scale;
            double posYMod = -(translatedYMod);
        ...
        double calculatedHeight = furtherstYBot - furtherstYTop;
        double calcuatedWidth = furtherstXRight - furtherstXLeft;

        double diffX = totalWidth - calcuatedWidth + furtherstXLeft;
        double diffY = totalHeight - calculatedHeight + furtherstYTop;
        mainCentralPanel.addComponent(centralPanel).inTL((float) -furtherstXLeft, (float) -furtherstYTop);
