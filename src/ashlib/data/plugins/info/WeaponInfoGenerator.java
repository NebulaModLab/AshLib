...
public class WeaponInfoGenerator {
    ...
    public static ArrayList<StatNumberUIPackage> getFirstRow(TooltipMakerAPI tooltip, WeaponSpecAPI spec, float padding) {
        ...
        String text3 = AshMisc.getNumberString(spec.getOrdnancePointCost(null));
    ...
    public static ArrayList<StatNumberUIPackage> getDelayRow(TooltipMakerAPI tooltip, WeaponSpecAPI spec, float padding) {
        ...
        if(spec.getTurretFireOffsets().size()>0){
            if(spec.getTurretFireOffsets().size()>1){
                text1 = AshMisc.getNumberString(spec.getBurstSize());
            ...
            text1 = AshMisc.getNumberString(delay);
    ...
    public static ArrayList<StatNumberUIPackage> getAmmoInfo(TooltipMakerAPI tooltip, WeaponSpecAPI spec, float padding) {
        ArrayList<StatNumberUIPackage> packages = new ArrayList<>();
            ...
            if(!spec.usesAmmo()){
                ...
                String text2 = null;
                String text3 = null;
                text1= AshMisc.getNumberString(spec.getMaxAmmo());
                text2 = AshMisc.getNumberString(spec.getReloadSize()/spec.getAmmoPerSecond());
                text3 = AshMisc.getNumberString(spec.getReloadSize());
    ...
    public static ArrayList<StatNumberUIPackage> getDamageRow(TooltipMakerAPI tooltip, WeaponSpecAPI spec, float padding) {
        ...
        String text1 = AshMisc.getNumberString(spec.getMaxRange()) + "";
        weaponStatsAPI.getSustainedDps();
        String text2 = AshMisc.getNumberString(weaponStatsAPI.getDamagePerShot()) + "";
        BeamWeaponSpecAPI specBeam = null;
        float delayBeam = 1f;
        if (spec.getBurstSize() > 1) {
            text2 = AshMisc.getNumberString(weaponStatsAPI.getDamagePerShot()) + "x" + spec.getBurstSize();
        }
        if (spec.isBeam()) {
        ...
        if (!spec.isNoDPSInTooltip()) {
            ...
            if (weaponStatsAPI.getDps()!=weaponStatsAPI.getSustainedDps()) {
                text3 = AshMisc.getNumberString(weaponStatsAPI.getDps()) + "(" + AshMisc.getNumberString(weaponStatsAPI.getSustainedDps()) + ")";
    ...
    public static ArrayList<StatNumberUIPackage> getFluxRow(TooltipMakerAPI tooltip, WeaponSpecAPI spec, float padding) {
        ...
        if (!spec.isNoDPSInTooltip()) {
            if (weaponStatsAPI.getDps()!=weaponStatsAPI.getSustainedDps()) {
                text1 = AshMisc.getNumberString(weaponStatsAPI.getFluxPerSecond()) + "(" + AshMisc.getNumberString(weaponStatsAPI.getSustainedFluxPerSecond()) + ")";
                packages.add(new StatNumberUIPackage(tooltip.addPara("Flux / second(sustained)", padding), text1, null, Color.ORANGE, null));
            } else   {
                text1 = AshMisc.getNumberString(weaponStatsAPI.getFluxPerSecond()) + "";
        ...
        if (!spec.isBeam()) {
            text1 = AshMisc.getNumberString(weapon.getFluxCostToFire() / spec.getBurstSize()) + "";
        ...
        if (weaponStatsAPI.getFluxPerDam() > 0) {
            text1 = AshMisc.getNumberString(weaponStatsAPI.getFluxPerDam());
